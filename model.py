...
def process_energy_data_from_excel(excel_path: str, api_key: str) -> str:
    ...
    # Filter and format data for energy demand by building
    building_demand = df[['building', 'demand']].groupby('building')['demand'].sum().reset_index()
    data_summary = building_demand.to_string(index=False)
